...
export const updateProfile = async (req, res) => {
    try {
        ...
        // Check if file is uploaded before attempting to process it
        let cloudResponse = null;
        if (file) {
            const fileUri = getDataUri(file);  // Get Data URI from file
        ...
        // Middleware sets userId in req.id, make sure it's set properly
        const userId = req.id;
        let user = await User.findById(userId);
        ...
        // If a file was uploaded, update resume fields
        if (cloudResponse) {
        ...
        // Create a sanitized response object (to avoid sending sensitive data)
        user = {
            _id: user._id,
            fullname: user.fullname,
            email: user.email,
